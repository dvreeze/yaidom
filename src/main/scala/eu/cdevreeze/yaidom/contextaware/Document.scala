...
final class Document(
  ...
  children: immutable.IndexedSeq[Nodes.CanBeDocumentChild]) extends ContextAwareDocument[simple.Elem](xmlDeclarationOption, children) with Immutable {
...
object Document {
  ...
  def from(d: simple.Document, uriResolver: XmlBaseSupport.UriResolver): Document = {
    val elem = ContextAwareScopedElem.Builder(classTag[simple.Elem], uriResolver).build(d.uriOption, d.documentElement)
