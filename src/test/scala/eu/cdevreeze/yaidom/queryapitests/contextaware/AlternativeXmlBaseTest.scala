...
/**
 * Alternative XML Base test case for contextaware Elems. This test uses the XML Base tutorial at: http://zvon.org/comp/r/tut-XML_Base.html.
 *
...
class AlternativeXmlBaseTest extends AbstractAlternativeXmlBaseTest {
  ...
  type E = yaidom.contextaware.Elem

  type E2 = yaidom.queryapi.ContextPath.Entry

  protected def convertToDocument(elem: yaidom.simple.Elem, docUri: URI): DocumentApi[E] = {
    val doc = yaidom.contextaware.Document.from(yaidom.simple.Document(Some(docUri), elem), resolveUri _)
    doc
  }
