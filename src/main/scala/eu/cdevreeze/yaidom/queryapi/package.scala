 ...
 * where the 1.0 `ElemApi` became `ElemApi with HasENameApi`.
 *
 ...
 * orthogonality of the query API has improved. For example, now we can abstract over `indexed` and `docaware`
 * elements by using the following type: `ElemApi[E] with HasPathApi with IsNavigableApi[E]`. This is a very
 * powerful type, supporting queries using predicates in which the elements themselves contain Paths (that can be
 * navigated to).
 *
