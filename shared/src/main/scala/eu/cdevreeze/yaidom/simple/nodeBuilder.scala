 ...
 * and [[eu.cdevreeze.yaidom.simple.Elem]] and [[eu.cdevreeze.yaidom.simple.ElemBuilder]] in particular. `Elem`s have (fixed, resolved) `Scope`s,
 ...
 * trees (as `NodeBuilder`s or directly as `Node`s) are built in a top-down manner. The [[eu.cdevreeze.yaidom.simple.ConverterToDocument]]s
 * in package `convert` recursively build `Elem`s in a top-down manner, possibly creating an `Elem`
