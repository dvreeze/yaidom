...
/**
 * Document, containing a lazy "indexed" document element.
 *
 * Note that class `LazyIndexedDocument` does not have any query methods for `Elem` instances. In particular, the `ElemApi` does not
 ...
 */
abstract class LazyIndexedDocument[U <: ScopedElemApi[U]](
  val xmlDeclarationOption: Option[XmlDeclaration],
  val children: immutable.IndexedSeq[Nodes.CanBeDocumentChild]) extends DocumentApi[LazyIndexedScopedElem[U]] with Immutable {
  ...
  require(
    children.collect({ case elm: LazyIndexedScopedElem[U] => elm }).size == 1,
    s"A document must have exactly one (LazyIndexedScopedElem) child element (${uriOption.map(_.toString).getOrElse("No URI found")})")

  require(documentElement.path == Path.Empty, "The document element must have the root Path")

  final def documentElement: LazyIndexedScopedElem[U] =
    children.collect({ case elm: LazyIndexedScopedElem[U] => elm }).head
  ...
  def withDocumentElement(newRoot: LazyIndexedScopedElem[U]): LazyIndexedDocument[U]

  /** Creates a copy, but with the new xmlDeclarationOption passed as parameter newXmlDeclarationOption */
  def withXmlDeclarationOption(newXmlDeclarationOption: Option[XmlDeclaration]): LazyIndexedDocument[U]
}
