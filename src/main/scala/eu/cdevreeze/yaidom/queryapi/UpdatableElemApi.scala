...
object UpdatableElemApi {
  ...
  trait FunctionApi[N, E <: N] extends IsNavigableApi.FunctionApi[E] {
    ...
    def childNodeIndex(thisElem: E, pathEntry: Path.Entry): Int

    def withChildSeqs(thisElem: E, newChildSeqs: immutable.IndexedSeq[immutable.IndexedSeq[N]]): E

    def withUpdatedChildren(thisElem: E, index: Int, newChild: N): E

    def withPatchedChildren(thisElem: E, from: Int, newChildren: immutable.IndexedSeq[N], replace: Int): E

    def plusChild(thisElem: E, index: Int, child: N): E

    def plusChild(thisElem: E, child: N): E

    def plusChildOption(thisElem: E, index: Int, childOption: Option[N]): E

    def plusChildOption(thisElem: E, childOption: Option[N]): E

    def plusChildren(thisElem: E, childSeq: immutable.IndexedSeq[N]): E

    def minusChild(thisElem: E, index: Int): E

    def updateChildElem(thisElem: E, pathEntry: Path.Entry)(f: E => E): E

    def updateChildElem(thisElem: E, pathEntry: Path.Entry, newElem: E): E

    def updateChildElemWithNodeSeq(thisElem: E, pathEntry: Path.Entry)(f: E => immutable.IndexedSeq[N]): E

    def updateChildElemWithNodeSeq(thisElem: E, pathEntry: Path.Entry, newNodes: immutable.IndexedSeq[N]): E

    def updateElemOrSelf(thisElem: E, path: Path)(f: E => E): E

    def updateElemOrSelf(thisElem: E, path: Path, newElem: E): E

    def updateElemWithNodeSeq(thisElem: E, path: Path)(f: E => immutable.IndexedSeq[N]): E

    def updateElemWithNodeSeq(thisElem: E, path: Path, newNodes: immutable.IndexedSeq[N]): E

    def updateChildElems(thisElem: E, pathEntries: Set[Path.Entry])(f: (E, Path.Entry) => E): E

    def updateChildElemsWithNodeSeq(thisElem: E, pathEntries: Set[Path.Entry])(f: (E, Path.Entry) => immutable.IndexedSeq[N]): E

    def updateElemsOrSelf(thisElem: E, paths: Set[Path])(f: (E, Path) => E): E

    def updateElems(thisElem: E, paths: Set[Path])(f: (E, Path) => E): E

    def updateElemsOrSelfWithNodeSeq(thisElem: E, paths: Set[Path])(f: (E, Path) => immutable.IndexedSeq[N]): immutable.IndexedSeq[N]

    def updateElemsWithNodeSeq(thisElem: E, paths: Set[Path])(f: (E, Path) => immutable.IndexedSeq[N]): E

    def updateChildElems(thisElem: E, f: (E, Path.Entry) => Option[E]): E

    def updateChildElemsWithNodeSeq(thisElem: E, f: (E, Path.Entry) => Option[immutable.IndexedSeq[N]]): E

    def updateTopmostElemsOrSelf(thisElem: E, f: (E, Path) => Option[E]): E

    def updateTopmostElems(thisElem: E, f: (E, Path) => Option[E]): E

    def updateTopmostElemsOrSelfWithNodeSeq(thisElem: E, f: (E, Path) => Option[immutable.IndexedSeq[N]]): immutable.IndexedSeq[N]

    def updateTopmostElemsWithNodeSeq(thisElem: E, f: (E, Path) => Option[immutable.IndexedSeq[N]]): E
  }
}
