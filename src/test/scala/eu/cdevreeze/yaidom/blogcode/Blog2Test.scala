...
/**
 * Code of yaidom blog 2 ("advanced yaidom querying"). The blog uses examples from the coursera course Introduction to Databases,
 * by Jennifer Widom.
 *
...
class Blog2Test extends Suite {
  ...
  private val pathToParentDir: java.io.File =
    (new java.io.File(classOf[Blog2Test].getResource("books.xml").toURI)).getParentFile

  /**
   * The code in this test can be copied to the "introductory example" section in the second article on yaidom.
   */
