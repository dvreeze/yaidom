...
class QueryTest extends Suite {
  ...
  @Test def testQueryAll() {
    ...
    expectResult(indexedBookstore.elem.findAllElemsOrSelf) {
      elems map { e => e.elem }
    }
  }
  ...
  @Test def testQueryBookTitles() {
    ...
    val bookTitles =
      (indexedBookstore \ havingLocalName("Book")) map { e => e getChildElem (_.localName == "Title") }
  ...
  @Test def testQueryTitles() {
    ...
    val titles =
      for (ch <- indexedBookstore.findAllChildElems) yield ch.getChildElem(EName("Title"))
  ...
  @Test def testQueryBookIsbns() {
    ...
    val isbns =
      for (book <- indexedBookstore filterChildElems havingLocalName("Book")) yield book.attribute(EName("ISBN"))
  ...
  @Test def testQueryCheapBooks() {
    ...
    val books =
      for {
        book <- indexedBookstore \ havingLocalName("Book")
  ...
  @Test def testQueryCheapBookAuthors() {
    ...
    val cheapBookElms =
      for {
        bookElm <- indexedBookstore \ havingLocalName("Book")
  ...
  @Test def testQueryMagazinesWithSameNameAsBook() {
    ...
    val magazines =
      for {
        magazine <- indexedBookstore \ (_.localName == "Magazine")
  ...
  @Test def testQueryBooksOrderedByPrice() {
    ...
    val books = {
      for {
        book <- indexedBookstore \ havingLocalName("Book") sortWith { cheaper _ }
        price = book.attribute(EName("Price")).toDouble
      } yield elem(
        qname = QName("Book"),
        children = Vector(
  ...
  @Test def testQueryBookPairsFromSameAuthor() {
    ...
    val pairs =
      for {
        book1 <- indexedBookstore \ havingLocalName("Book")
        book2 <- indexedBookstore \ havingLocalName("Book")
