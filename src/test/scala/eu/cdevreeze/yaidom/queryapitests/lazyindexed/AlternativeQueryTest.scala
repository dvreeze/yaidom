...
import eu.cdevreeze.yaidom.indexed.LazyDocument
...
/**
 * Alternative query test case for lazy indexed Elems.
 *
...
class AlternativeQueryTest extends AbstractAlternativeQueryTest {
  ...
  protected val catalogElem: E = {
    ...
    val result = LazyIndexedScopedElem[simple.Elem](Some(new URI("http://catalog")), convertToElem(xml))

    // Invoking some Document methods, but without altering the result

    val doc = LazyDocument(result)
  ...
  protected val pricesElem: E = {
    ...
    LazyIndexedScopedElem[simple.Elem](Some(new URI("http://prices")), convertToElem(xml))
  }
  ...
  protected val orderElem: E = {
    ...
    LazyIndexedScopedElem[simple.Elem](Some(new URI("http://order")), convertToElem(xml))
  }
  ...
  protected def fromScalaElem(elem: scala.xml.Elem): E = {
    LazyIndexedScopedElem[simple.Elem](Some(new URI("http://bogus-uri")), convertToElem(elem))
  }
}
