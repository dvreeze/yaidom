 ...
 * <ul>
 * <li>attempt to offer XPath-like querying, thus somewhat blurring the distinction between nodes and node collections</li>
 ...
 *
 * Yaidom chose its battles. For example, knowing that DTDs do not know about namespaces, yaidom offers good namespace
 ...
 * at play here:
 * <ul>
 * <li><em>qualified names</em>, such as `book:Title` and `Edition`</li>
 * <li><em>expanded names</em>, such as `{http://bookstore/book}Title` (in James Clark notation) and `Edition`</li>
 * </ul>
 ...
 * declarations of the last `book:Book` element and of its ancestor elements, starting with the root element.
 *
 ...
 * }}}
 * We find no other namespace declarations in the ancestry of the last `book:Book` element, so the computed scope is also the scope
 ...
 * otherwise the index numbers may not match. Also note that the prefixes `book` and `auth` in the element path builder are
 ...
 *
 * The `ElemLike` trait has sub-trait [[eu.cdevreeze.yaidom.PathAwareElemLike]]. It adds knowledge about element paths. Element
 ...
 *
 * For example, to query for the element paths of the above-mentioned Scala book authors (relative to the root element), the following
 * code can be used (if the used element implementation mixes in trait `PathAwareElemLike`, which is not the case for the Scala XML and
 * DOM wrappers above):
 * {{{
 * for {
 ...
 * } yield authorElemPath
 * }}}
 *
 * The `PathAwareElemLike` trait has sub-trait [[eu.cdevreeze.yaidom.UpdatableElemLike]]. This trait offers functional updates
 ...
 *
 * Classes `Elem` and `ElemBuilder`, like their super-classes `Node` and `NodeBuilder`, have very much in common. Both are immutable,
