...
/**
 * Query test case. This test case is much like the "original" QueryTest, yet taking DOM wrapper elements instead of standard yaidom
 * Elems.
 *
...
class QueryTest extends Suite {
  ...
  @Test def testQueryCheapBookTitles() {
    ...
    val titles3 = bookstore.findAllElems filter { e =>
  ...
  @Test def testQueryBookPairsFromSameAuthor() {
    ...
    def bookAuthorLastNames(book: DomElem): Set[String] = {
    ...
    def bookTitleOfDomElem(book: DomElem): String = book.getChildElem(EName("Title")).trimmedText

    def bookTitleOfElem(book: Elem): String = book.getChildElem(EName("Title")).trimmedText
    ...
    val pairs =
      for {
        ...
        if bookTitleOfDomElem(book1) < bookTitleOfDomElem(book2)
      } yield elem(
        qname = QName("BookPair"),
        children = Vector(
          textElem(QName("Title1"), bookTitleOfDomElem(book1)),
          textElem(QName("Title2"), bookTitleOfDomElem(book2)))).build()
    ...
    expectResult(3) {
      pairs.filter(pair =>
        pair.getChildElem(EName("Title1")).trimmedText == bookTitleOfElem(book1.build()) ||
          pair.getChildElem(EName("Title2")).trimmedText == bookTitleOfElem(book1.build())).size
    }
    expectResult(3) {
      pairs.filter(pair =>
        pair.getChildElem(EName("Title1")).trimmedText == bookTitleOfElem(book2.build()) ||
          pair.getChildElem(EName("Title2")).trimmedText == bookTitleOfElem(book2.build())).size
    }
    expectResult(2) {
      pairs.filter(pair =>
        pair.getChildElem(EName("Title1")).trimmedText == bookTitleOfElem(book3.build()) ||
          pair.getChildElem(EName("Title2")).trimmedText == bookTitleOfElem(book3.build())).size
    }
    expectResult(2) {
      pairs.filter(pair =>
        pair.getChildElem(EName("Title1")).trimmedText == bookTitleOfElem(book4.build()) ||
          pair.getChildElem(EName("Title2")).trimmedText == bookTitleOfElem(book4.build())).size
    }
  }
