...
/**
 * Query test case for contextaware Elems.
 *
...
class QueryTest extends AbstractElemLikeQueryTest {
  ...
  @Test def testQueryAll(): Unit = {
    ...
    assertResult(true) {
      elems forall { e =>
        val parentScope = e.parentContextPath.lastEntryOption.map(_.scope).getOrElse(Scope.Empty)
        val namespaces = parentScope.relativize(e.contextPath.lastEntry.scope)
        parentScope.resolve(namespaces) == e.scope
      }
    }
  ...
  @Test def testQueryBookOrMagazineTitlesUsingParent(): Unit = {
    ...
    val bookOrMagazineTitles =
      for {
        ...
        if (title.parentContextPath.lastEntryOption.map(_.resolvedName) == Some(EName("Book"))) ||
          (title.parentContextPath.lastEntryOption.map(_.resolvedName) == Some(EName("Magazine")))
      } yield title
  ...
  @Test def testQueryTitlesAgain(): Unit = {
    ...
    val titles =
      for {
        ...
        if title.parentContextPath.entries.size == 2
      } yield title
    ...
    assertResult(Set(
      ...
      "National Geographic",
      "Newsweek")) {
      val result = titles map { e => e.trimmedText }
      result.toSet
    }
  }
  ...
  @Test def testQueryTitlesOfCheapBooksByUllmanUsingParent(): Unit = {
    ...
    val bookTitles =
      bookstore findTopmostElems { e => e.localName == "Last_Name" && e.trimmedText == "Ullman" } flatMap { elm =>
        ...
        val bookOption = elm.contextPath findAncestorContextPath { cp =>
          !cp.isEmpty && cp.lastEntry.resolvedName == EName("Book") && cp.lastEntry.attribute(EName("Price")).toInt < 90
        } flatMap { cp => bookstore.findElemOrSelf(_.contextPath == cp) }
  ...
  @Test def testQueryTitlesOfCheapBooksByJeffreyUllmanUsingParent(): Unit = {
    ...
    val bookTitles2 =
      for {
        ...
        bookElem <- authorElem.contextPath findAncestorContextPath { _.lastEntryOption.map(_.resolvedName) == Some(EName("Book")) } flatMap { cp =>
          bookstore.findElemOrSelf(_.contextPath == cp)
        }
  ...
  @Test def testQueryBooksByJeffreyUllmanUsingParent(): Unit = {
    ...
    val ullmanBookElms =
      for {
        ...
        bookElmContextPath = authorElm.parentContextPath.parentContextPath
        bookElm <- bookstore.findElemOrSelf(_.contextPath == bookElmContextPath)
      } yield {
  ...
  @Test def testQueryElementsWithParentNotBookOrBookstore(): Unit = {
    ...
    val elms =
      for {
        ...
        parentContextPath = e.parentContextPath
        if parentContextPath.lastEntry.qname != QName("Bookstore") && parentContextPath.lastEntry.qname != QName("Book")
      } yield e
  ...
  @Test def testAncestryQueryingConsistency(): Unit = {
    val reverseAncestryOrSelfENames = bookstore.findAllElemsOrSelf.map(_.contextPath.entries.map(_.resolvedName))
    ...
    val reverseAncestryOrSelf = bookstore.findAllElemsOrSelf.map(_.contextPath)

    assertResult(reverseAncestryOrSelfENames) {
      reverseAncestryOrSelf.map(_.entries.map(_.resolvedName))
    }
  }
