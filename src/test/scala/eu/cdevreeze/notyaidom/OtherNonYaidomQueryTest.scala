...
/**
 * Query test case, much like `NonYaidomQueryTest`, but experimenting with the use of member types instead of
 * type parameters, where we bind the member types as late as possible (only in the concrete element implementation).
 *
 * Maybe such a design with member types could be used in yaidom 2.
 *
...
class OtherNonYaidomQueryTest extends Suite {
  ...
  @Test def testQueryTitlesOfBooksWithRemarks(): Unit = {
    ...
    val bookTitles =
      for {
        book <- bookstore filterChildElems { _.name == "Book" }
        if !book.filterChildElems(_.name == "Remark").isEmpty
      } yield book.filterChildElems(_.name == "Title").head
  ...
  @Test def testQueryTitlesOfCheapBooksByUllman(): Unit = {
    ...
    val bookTitles =
      for {
        ...
        authors = book.filterChildElems(_.name == "Authors").head
  ...
  @Test def testQueryBooksOrMagazinesWithTitleAsOtherBook(): Unit = {
    ...
    val booksAndMagazines =
      for {
        bookOrMagazine <- bookstore filterChildElems { e => Set("Book", "Magazine").contains(e.name) }
        titleString: String = bookOrMagazine.findAllChildElems.find(_.name == "Title").get.text.trim
        otherBooks = bookstore.filterChildElems(_.name == "Book").toSet -- Set(bookOrMagazine)
  ...
  @Test def testQueryBookPairsFromSameAuthor(): Unit = {
    ...
    assertResult(3) {
      pairs.filter(pair =>
        pair.filterChildElems(_.name == "Title1").head.text.trim == bookTitle(book1) ||
          pair.filterChildElems(_.name == "Title2").head.text.trim == bookTitle(book1)).size
    }
    assertResult(3) {
      pairs.filter(pair =>
        pair.filterChildElems(_.name == "Title1").head.text.trim == bookTitle(book2) ||
          pair.filterChildElems(_.name == "Title2").head.text.trim == bookTitle(book2)).size
    }
    assertResult(2) {
      pairs.filter(pair =>
        pair.filterChildElems(_.name == "Title1").head.text.trim == bookTitle(book3) ||
          pair.filterChildElems(_.name == "Title2").head.text.trim == bookTitle(book3)).size
    }
    assertResult(2) {
      pairs.filter(pair =>
        pair.filterChildElems(_.name == "Title1").head.text.trim == bookTitle(book4) ||
          pair.filterChildElems(_.name == "Title2").head.text.trim == bookTitle(book4)).size
    }
  }
...
object OtherNonYaidomQueryTest {
  ...
  trait AnyElem {
    ...
    /**
     * The "self type" of the element.
     */
    type ThisElem <: AnyElem
  }
