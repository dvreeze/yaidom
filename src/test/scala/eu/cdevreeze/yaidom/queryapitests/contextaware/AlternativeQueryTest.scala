...
/**
 * Alternative query test case for contextaware Elems.
 *
...
class AlternativeQueryTest extends AbstractAlternativeQueryTest {
  ...
  protected val catalogElem: E = {
    ...
    val result = contextawareElemBuilder.build(Some(new URI("http://catalog")), convertToElem(xml))
  ...
  protected val pricesElem: E = {
    ...
    contextawareElemBuilder.build(Some(new URI("http://prices")), convertToElem(xml))
  }
  ...
  protected val orderElem: E = {
    ...
    contextawareElemBuilder.build(Some(new URI("http://order")), convertToElem(xml))
  }
  ...
  protected def fromScalaElem(elem: scala.xml.Elem): E = {
    contextawareElemBuilder.build(Some(new URI("http://bogus-uri")), convertToElem(elem))
  }
}
