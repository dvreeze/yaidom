...
/**
 * Query test case. This test case is much like the "original" QueryTest, yet taking "resolved" Elems instead of standard yaidom
 * Elems.
 *
...
class QueryTest extends Suite {
  ...
  @Test def testQueryAllElements() {
    ...
    expectResult(elements) {
      paths map { path => bookstore.getWithElemPath(path) }
    }
  }
  ...
  @Test def testQueryElementsWithParentNotBookOrBookstoreUsingStoredElemPaths() {
    ...
    def addElemPaths(e: Elem, path: ElemPath, scope: Scope): Elem = {
      ...
      Elem(
        ...
        resolvedAttributes = e.resolvedAttributes + (EName("elemPath") -> path.toCanonicalXPath(scope)),
    ...
    expectResult(Set(EName("Title"), EName("Author"), EName("First_Name"), EName("Last_Name"))) {
      val result = paths map { path => bookstore.getWithElemPath(path).resolvedName }
      result.toSet
    }
  }
  ...
  @Test def testQueryBookPairsFromSameAuthor() {
    ...
    val pairs =
      for {
        ...
        resolvedName = EName("BookPair"),
        resolvedAttributes = Map(),
        children = Vector(
          Elem(EName("Title1"), Map(), Vector(Text(bookTitle(book1)))),
          Elem(EName("Title2"), Map(), Vector(Text(bookTitle(book2))))))
    ...
    expectResult(3) {
      pairs.filter(pair =>
        pair.getChildElem(EName("Title1")).trimmedText == bookTitle(resolved.Elem(book1.build())) ||
          pair.getChildElem(EName("Title2")).trimmedText == bookTitle(resolved.Elem(book1.build()))).size
    }
    expectResult(3) {
      pairs.filter(pair =>
        pair.getChildElem(EName("Title1")).trimmedText == bookTitle(resolved.Elem(book2.build())) ||
          pair.getChildElem(EName("Title2")).trimmedText == bookTitle(resolved.Elem(book2.build()))).size
    }
    expectResult(2) {
      pairs.filter(pair =>
        pair.getChildElem(EName("Title1")).trimmedText == bookTitle(resolved.Elem(book3.build())) ||
          pair.getChildElem(EName("Title2")).trimmedText == bookTitle(resolved.Elem(book3.build()))).size
    }
    expectResult(2) {
      pairs.filter(pair =>
        pair.getChildElem(EName("Title1")).trimmedText == bookTitle(resolved.Elem(book4.build())) ||
          pair.getChildElem(EName("Title2")).trimmedText == bookTitle(resolved.Elem(book4.build()))).size
    }
  }
  ...
  private val bookstore: resolved.Elem = {
    import NodeBuilder._
